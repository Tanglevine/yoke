...
public class Router extends AbstractMiddleware {
    ...
    private void init(Yoke yoke, String mount, List<PatternBinding> bindings) {
        for (PatternBinding binding : bindings) {
            binding.middleware.stream().filter(m -> m instanceof AbstractMiddleware).forEach(m -> {
                if (!((AbstractMiddleware) m).isInitialized()) {
    ...
    public Router param(@NotNull final String paramName, @NotNull final Middleware handler) {
        ...
        if (handler instanceof AbstractMiddleware) {
            if (!((AbstractMiddleware) handler).isInitialized() && isInitialized()) {
    ...
    private void addPattern(String verb, String input, Middleware[] handler, List<PatternBinding> bindings) {
        ...
        for (Middleware h : handler) {
            if (h instanceof AbstractMiddleware) {
                if (!((AbstractMiddleware) h).isInitialized() && isInitialized()) {
